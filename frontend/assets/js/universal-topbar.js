    ...
    class UniversalTopBar {
        ...
        createTopBar() {
            ...
            topbar.innerHTML =
                ...
                '<a href="/" class="topbar-logo"><i class="fas fa-phone"></i><span>Softstock</span></a>' +
                '</div>' +
        ...
        async getUser() {
            ...
            if (window.globalAuth && window.globalAuth.isLoggedIn()) {
                ...
                var avatar = null;
                if (user && user.email) avatar = await this.loadProfilePhotoFromSupabase(user.email);
                if (!avatar && user) avatar = user.profile_photo || user.profilePhoto || null;
                if (!avatar) avatar = getAvatarFallback(name, user ? user.email : '');
                return { name: name, avatar: avatar, email: user ? user.email : null, isLoggedIn: true };
            }

            // 2. Fall back to raw localStorage — covers iamcalling_uid / uid flows
            try {
                var raw = localStorage.getItem('currentUser');
                var cu = raw ? JSON.parse(raw) : null;
                var uid = localStorage.getItem('iamcalling_uid') || localStorage.getItem('uid') || (cu && cu.id ? String(cu.id) : null);
                if (uid && cu) {
                    var n = cu.full_name || cu.fullName ||
                            ((cu.first_name || cu.firstName || '') + ' ' + (cu.last_name || cu.lastName || '')).trim() ||
                            cu.name || cu.email || 'User';
                    var ph = cu.profile_photo || cu.profilePhoto || null;
                    if (!ph && cu.email) ph = await this.loadProfilePhotoFromSupabase(cu.email);
                    if (!ph) ph = getAvatarFallback(n, cu.email);
                    return { name: n, avatar: ph, email: cu.email || null, isLoggedIn: true };
                }
            } catch(e) {}
        ...
        async updateUserDisplay() {
            ...
            // Not logged in — show both Login and Register buttons
            display.innerHTML =
                '<a href="15-login.html" class="login-btn-topbar" style="margin-right:6px;">Login</a>' +
                '<a href="16-register.html" style="color:#000;background:linear-gradient(135deg,#d4af37,#f0c040);font-weight:700;padding:8px 16px;border-radius:5px;text-decoration:none;font-size:14px;">Register</a>';
        }
