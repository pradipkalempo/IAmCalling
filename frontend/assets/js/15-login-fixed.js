    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Build normalized user object — always snake_case to match DB
                const userData = {
                    id:            user.id,
                    email:         user.email,
                ...
                // Also store uid separately as a stable session key
                localStorage.setItem('uid', userData.id);

                // Also notify globalAuth if available
                if (window.globalAuth) window.globalAuth.setUser(userData);
